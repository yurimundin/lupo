...
**Lupo** é um gerenciador de senhas desktop para Windows, **open source
...
**Lupo preenche o espaço** entregando, no mesmo produto:
...
**Identifier do app (Tauri / Microsoft Store):** `br.app.basis.lupo`
...
3. **Compatibilidade total com .kdbx.** Um arquivo gerado pelo Lupo
...
- [ ] **Mobile (Android/iOS) via Tauri Mobile.** Exige reengenharia de UX
      ...
      Pode virar **projeto irmão** (`Lupo Mobile`) em vez de feature
...
**Decisão (sessão 2):** toda derivação de chave Argon2 do Lupo roda no
...
5. **Desktop-only.** Lupo não precisa rodar em browser, então perder
...
**Decisão (2026-05-04):** o Lupo NÃO terá servidor de sincronização
...
**Implicação:** propostas/issues pedindo "Lupo Cloud" devem ser
...
`localStorage` (`lupo-settings`) sem criptografia. Trata o caminho
...
- [src-tauri/src/lib.rs](src-tauri/src/lib.rs) → `log_smoke_result` agora
  também grava em `%TEMP%/lupo-bench.log` (em release o `println!`
...
- `target/release/bundle/msi/Lupo_0.1.0_x64_en-US.msi`: **3,40 MB**
- `target/release/bundle/nsis/Lupo_0.1.0_x64-setup.exe`: **2,26 MB**
...
`%TEMP%/lupo-bench.log` ficam permanentes como utilidade de debug
...
- `npm run tauri dev` validado: compilação Rust completa em ~2m20s na
  ...
  Lupo abriu corretamente (validação visual pelo Yuri).
...
- [src/lib/theme.ts](src/lib/theme.ts) com `initTheme/setTheme`, suporta
  preferência manual (`localStorage["lupo-theme"]`) ou
...
- `[package].name = "lupo"`, `lib.name = "lupo_lib"`,
  `main.rs` chama `lupo_lib::run()`.
...
- [src/stores/settings.ts](src/stores/settings.ts) (com `persist`,
  chave `lupo-settings`): `autoLockMs` (default 5 min),
...
Repositório público: <https://github.com/yurimundin/lupo>
...
- ✅ **Sessão 8 — Redesign tela de unlock + auto-open último cofre**
  (`a08f14f`):
  - Logo Lupo 96px substitui ícone de cadeado pequeno.
...
- ✅ **Sessão 10 — Updates de dependências + documentação smoke test**
  ...
  - Migração do projeto de `Downloads/` para `C:\dev\lupo` (Smart
...
- ✅ **Sessão 14 — DevDep vulnerability + lição §30** (`2575be6`):
  ...
  - Vulnerability não é alcançável em runtime do Lupo (devDep,
...
- ✅ **Sessão 15 — Smart App Control gotcha + housekeeping documental
  ...
  - Erro `os error 4551` em `tauri dev` rastreado até Smart App Control
    (SAC) bloqueando build-scripts e o próprio `lupo.exe` quando
...
- ✅ **Sessão 19 — Quality + alpha distribution prep** (4 commits):
  ...
  - **Bloco 4** (`6286099`) — alpha .exe-only Windows distribution:
    `identifier` mudou de `com.lupo.app` (placeholder) para
    `br.app.basis.lupo` (reverse-DNS de `basis.app.br`); `bundle.targets`
    cravado em `["msi"]` para uso futuro; `bundle.active: false`
    desativa bundling MSI (que rejeita pre-release SemVer `-alpha`),
    `tauri build` agora produz só `lupo.exe` 12 MB. Para reativar
...
- ✅ **Sessão 21 — Powered by BasisApp + housekeeping S20** (2 commits):
  ...
  - **Bloco 2** (`062b4c1`) — Powered by BasisApp + refactor
    ...
    text-muted-foreground` linkando `lupo.basis.app.br`); helper
  ...
  - Decisões UX: sidebar bottom (não window footer ou About);
    ...
    dialog interno); URL `lupo.basis.app.br` (não basis.app.br raiz)
...
- ✅ **Sessão 23 — Alpha distributable for testers** (`v0.1.0-alpha`):
  - Build fresh do `lupo.exe` (12 MB, compilado de `0f46473`).
  ...
  - ZIP profissional `Lupo-0.1.0-alpha-windows-x64.zip` (3.85 MB,
    compressão 67%) com 4 arquivos: lupo.exe + LEIAME.txt (89 linhas
  ...
  - Release URL: https://github.com/yurimundin/lupo/releases/tag/v0.1.0-alpha
...
mas a UI do Lupo é reativa via `vaultVersion`. Toda função que
...
1. **Compatibilidade total com KeePass/KeePassXC.** Cofre alterado pelo
   Lupo abre no KeePassXC com a Lixeira no lugar certo, e vice-versa.
...
usa internamente), entries restauradas no Lupo aparecem
...
- Descrição: lembra que **Lupo não consegue desfazer** e sugere
...
**Versão atual:** `0.1.7`. Release Windows de validação controlada, ainda sem
...
Conteúdo estático: logo (128×128 servido de `public/lupo-appicon-steel.svg`,
...
"Sobre o Lupo". `aria-label` espelha o tooltip.
...
`public/lupo-appicon-steel.svg` é cópia direta de
`src-tauri/icons/128x128.png`. Quando o ícone fonte for atualizado
(via `npx @tauri-apps/cli icon lupo-appicon-steel.svg`,
...
**Logo:** `<img src="/lupo-appicon-steel.svg" />` 96×96px (`h-24 w-24`)
substitui o ícone `<Lock />` pequeno usado até a Sessão 7. Fonte
servida do `public/lupo-appicon-steel.svg` (cópia de
...
**só são compiladas em build Linux**. Lupo hoje só faz build para
...
**Contexto:** Lupo valida no boot que o pipeline cripto (Argon2id
...
`%TEMP%/lupo-bench.log` (silencioso se falhar). Esse arquivo
...
sidebar do Lupo até a Sessão 10 só renderizava filhos diretos
...
- **Ícones e cores de grupos:** cada nó também carrega `iconId` e
  `iconColorId`, lidos de `customData` com chaves próprias do Lupo
  (`lupo.groupIcon` e `lupo.groupIconColor`, com leitura legada de
  `sec.basis.groupIcon`/`sec.basis.groupIconColor` para cofres antigos).
  A UI usa ícones Lucide, incluindo `Scale` para a opção "Jurídico", e uma
  paleta pequena de cores nomeadas. Isso não altera o `IconID` nativo do
  KeePass; outros clientes ignoram esses metadados e continuam abrindo o
  cofre.
...
**Decisão de processo Lupo:** Caminho 1 (fechar). Atraso aceitável
...
- **Alerta dispensado (Sessão 14):** GHSA em `ip-address < 10.1.1`
  ...
  `ip-address@10.1.0`. Lupo não importa `ip-address` em runtime;
  ...
  code not reachable from Lupo runtime, devDep does not ship in
  binary".
...
- **Sessão 9.5/10 (máquina nova Windows 11):** primeiro encontro com
  ...
  mover de `Downloads/03 - Projetos/Code_Playground/lupo` para
  `C:\dev\lupo`. Build voltou a funcionar.
...
Lupo é o primeiro produto sob o domínio `basis.app.br` (família
de aplicações). Decisões de empacotamento são tomadas considerando
que:

- Lupo é e permanecerá 100% FOSS gratuito, sem versão paga,
...
- **Canais primários:** GitHub Releases + `lupo.basis.app.br`.
...
6. **Smart App Control e build de release.** §31 documenta
   ...
   mesmo problema. Pré-requisito: mover projeto para `C:\dev\lupo`
...
1. Mover projeto para `C:\dev\lupo` (se ainda em `Downloads/`)
...
- **Custo do cert renovando anualmente.** Lupo sem monetização
...
é prudência, não procrastinação. Lupo em alpha/beta pode rodar
