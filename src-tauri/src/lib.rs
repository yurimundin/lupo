...
/// Inspeciona arquivos de recuperacao ao lado do cofre.
///
/// `.bak` sozinho e normal depois de um save bem-sucedido, entao so
/// marcamos `needs_attention` quando ha `.tmp` pendente ou quando o cofre
/// atual parece invalido mas ha backup valido para restaurar.
#[tauri::command]
fn inspect_vault_recovery(file_path: String) -> Result<VaultRecoveryState, String> {
...
/// Restaura `<cofre>.kdbx.bak` sobre o cofre atual, preservando o estado
/// anterior em `<cofre>.kdbx.before-restore-<timestamp>.bak`.
#[tauri::command]
fn restore_vault_backup(file_path: String) -> Result<RestoreBackupResult, String> {
    if file_path.trim().is_empty() {
        return Err("Caminho do cofre vazio.".to_string());
    }
    if !file_path.to_lowercase().ends_with(".kdbx") {
    ...
    let bak_bytes = std::fs::read(&bak_path)
        .map_err(|e| io_error_to_pt(&e, &bak_path, "ler backup"))?;
...
fn recovery_file_info(path: &str) -> RecoveryFileInfo {
    ...
    RecoveryFileInfo {
        ...
        has_kdbx_magic: bytes.as_ref().is_some_and(|file_bytes| has_kdbx_magic(file_bytes)),
    }
}

fn has_kdbx_magic(bytes: &[u8]) -> bool {
